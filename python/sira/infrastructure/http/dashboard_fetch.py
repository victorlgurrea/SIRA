...
def bootstrap_dashboard_data(api_base: str) -> dict:
    """Arranque: snapshot GitHub + API si está más reciente."""
    global _stale
    ...
    # Si la API ya ingirió después del snapshot, usarla (evita pintar el JSON de ayer).
    chosen = load_dashboard_payload(api_base)
    if chosen.get("generado_en") and _payload_score(chosen) > 0:
        _stale = chosen
        log.info(
            "Bootstrap: generado_en=%s score=%d",
            chosen.get("generado_en"),
            _payload_score(chosen),
        )
        return chosen
    if isinstance(local, dict) and local.get("generado_en") and _payload_score(local) > 0:
        _stale = local
        return local
...
def _gen_ts(value: object) -> str:
    return str(value or "")


def load_dashboard_payload(api_base: str) -> dict:
    """Prefiere API si su generado_en es más nuevo; si no, snapshot/disco.

    /api/status es ligero (no timeout). Solo descarga /api/dashboard cuando hace
    falta (API más reciente o disco vacío).
    """
    global _stale

    local = _maybe_refresh_snapshot(read_dashboard())
    local_gen = _gen_ts((local or {}).get("generado_en") if isinstance(local, dict) else None)

    status = fetch_status_api(api_base)
    api_gen = _gen_ts((status or {}).get("generado_en") if isinstance(status, dict) else None)

    # Disco ya al día respecto a la API: no bajar el JSON pesado otra vez.
    if local_gen and (not api_gen or api_gen <= local_gen) and _payload_score(local) > 0:
        _stale = local
        return local

    need_api = (not local_gen) or (bool(api_gen) and api_gen > local_gen)
    if need_api:
        fresh = _fetch_dashboard_api(api_base)
        if fresh and _payload_score(fresh) > 0:
            try:
            ...
            _stale = fresh
            log.info(
                "Dashboard actualizado desde API: %s → %s",
                local_gen or "—",
                fresh.get("generado_en"),
            )
            return fresh
        log.warning(
            "API status generado_en=%s más reciente que disco=%s, pero /api/dashboard no respondió",
            api_gen or "—",
            local_gen or "—",
        )

    chosen = _pick_fresher(local, _stale)
    if chosen.get("generado_en") and _payload_score(chosen) > 0:
        _stale = chosen
        return chosen
    return local if isinstance(local, dict) else {}
